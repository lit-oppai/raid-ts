    ...
    <div class="">
        ...
        <div v-for="[label, disk] in allNewDiskStatus" :key="label">
            <div class="mt-6 mb-2">
                <span
                    v-t="{
                        path: `DiscoverNewHardDrive.driveBay{label}`,
                        args: { label },
                    }"
            ...
            <div class="w-full bg-white rounded-lg px-4 py-3">
                ...
                <div class="w-full flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-1">
                        <span
                            class="text-zinc-800 text-base font-medium font-['Roboto']"
                        >
                            {{ disk.model }}
                        </span>
                        ...
                        <span
                            v-t="
                                `DiscoverNewHardDrive.requiresFormattingReminder.${!disk.support}`
                            "
                    ...
                    ></Button>
                    <Button
                        :label="$t('Format and Enable')"
                        severity="primary"
                        size="medium"
                        @click="showCheckFormat(disk.type, disk.path)"
                        v-else
                    ></Button>
                </div>
                <!-- Content -->
                <div
                    class="ml-9 mt-4 bg-gray-50 rounded-lg p-3 gap-2 flex flex-col"
                    v-if="disk?.children_number ?? 0 > 0"
                >
                    <!-- Traversing Part -->
                    <div
                        class="rounded-md border border-gray-200 px-1.5 flex items-center h-9"
                        v-for="(item, key) in disk.children"
                        :key="key"
                    >
                        <span
                            class="text-zinc-800 text-xs font-medium font-['Roboto'] px-2"
                        >
                            {{ item?.name }}
                        </span>
                        ...
                        >
                            {{
                                $t("DiscoverNewHardDrive.item.PreservingData")
                            }}
                            · {{ convertSizeToReadable(item?.size) }}
                        </span>
                        <span
                        ...
                        >
                            {{
                                $t(
                                    "DiscoverNewHardDrive.item.NeedsToBeFormatted",
                                )
                            }}
                            · {{ convertSizeToReadable(item?.size ?? 0) }}
                        </span>
                        <!-- paperwork end -->
                        ...
                        <Button
                            ...
                            severity="secondary"
                            size="small"
                            @click="showCheckFormat(disk.type, item?.path)"
                            v-else
                        ></Button>
                        <!-- paperwork end -->
                    </div>
