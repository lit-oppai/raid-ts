...
const loadNewDiskStatus = async () => {
    await disk.getDisks("show").then((res) => {
        newDiskStatus.value = res.data.data?.filter((item) => item?.free && item?.health === 'true');
    })
}

// 最小的盘尺寸
// 获取选择的硬盘
import { selectedFidDisk, needMinNewDiskSize } from "@views/EstablishRAID/controlData.ts";
onMounted(async () => {
    await loadNewDiskStatus();
    selectedFidDisk.value = newDiskStatus.value?.find((item) => item.size && item.size >= needMinNewDiskSize.value)?.path ?? '';
})
</script>
    ...
    <template v-if="true">
        ...
        <div class="flex flex-col gap-2">
            ...
            <label class="flex items-center h-10 bg-white rounded-md border border-neutral-300 gap-2 px-3 cursor-pointer"
                v-for="item, index in newDiskStatus" :key="index" :for="`select${index}`"
                :class="{ 'opacity-50': item && item.size && needMinNewDiskSize > item.size }">
                ...
                <span class="align-baseline text-zinc-800 text-sm font-medium font-['Roboto']">
                    {{ item.model }} - {{ convertSizeToReadable(item.size as number) }}
                </span>
