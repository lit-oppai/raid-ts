...
const checkNextStep = computed<boolean>(() => {
    ...
    if (currentStepName.value === 'SelectRAIDPart') {
        switch (selectRAIDStrategy.value) {
            ...
            case 'RAID5':
                return diskListByStorageSpace.value.length ? selectStorageList.value.length < 1 : selectStorageList.value.length < 3;
        }
...
const confirmFirstAid = () => {
    stepByStep('next');
    openAPI.raid.updateRaid({ devices: [selectedFidDisk.value as string], path: needFirstAidRaid.value, action: 'add' }).then((res) => {
        resultRAIDInfo.capacity = (res.data.size ?? 0 - 0) * 1024;
        resultRAIDInfo.btnText = 'Done';
        resultRAIDInfo.success = true;
        resultRAIDInfo.butFunc = () => {
...
const extendCapacity = () => {
    stepByStep('next');
    openAPI.raid.updateRaid({ devices: pathList.value, path: extendRaidPath.value ?? '', action: 'add' }).then((res) => {
        resultRAIDInfo.capacity = (res.data.size ?? 0 - 0) * 1024;
        resultRAIDInfo.btnText = 'Done';
        resultRAIDInfo.success = true;
        resultRAIDInfo.butFunc = () => {
...
<template>
    <div class="space-x-4">
        ...
        <Button label="Cancel" severity="neutral" size="medium" @click="closeEstablishRAID"
            v-show="context !== 'FirstAid' && currentStepName !== 'OverviewPart' && context === 'Modify' && currentStep < 2"></Button>

        <Button label="Previous" severity="neutral" size="medium" @click="stepByStep('prev')"
            v-show="context !== 'FirstAid' && currentStepName !== 'ResultRAIDPart' && currentStepName !== 'ResultRAIDPart'"></Button>
        <Button label="Next" severity="primary" size="medium" @click="stepByStep('next')"
            v-show="context !== 'FirstAid' && currentStepName !== 'ConfirmRAIDPart' && currentStepName !== 'ResultRAIDPart'"
