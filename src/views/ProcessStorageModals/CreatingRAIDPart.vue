...
switch (context.value) {
    case "FirstAid":
        ...
        break;
    case "CreateStorage":
        content = onlyFormatSingleStorageSpace.value
            ? `${t("Formatting")}...`
            : `${t("Creating")}...`;
        break;
    default:
...
<template name="CreatingRAIDPart">
    <div class="flex flex-col items-center space-y-2 pt-[5%]">
        <LottieAnimation
            :animation-data="creatingJSON"
            loop
            class="w-[6.25rem] h-[6.25rem]"
        />
        <LottieAnimation
            :animation-data="EasterEgg"
            loop
            v-if="showEasterEgg"
        />
    ...
    <div class="mt-3 mx-6">
        <span class="text-neutral-400 text-sm font-normal font-['Roboto']">
            {{
                $t(
                    "During this process, please do not unplug the hard drive or shut down ZimaCube to avoid system errors.",
                )
            }}
        </span>
    </div>
